...
export const RequestBuilder: React.FC<RequestBuilderProps> = ({ tabId }) => {
  ...
  // 可调整大小的分隔条 - 默认给响应更多空间
  const [upperHeight, setUpperHeight] = useState(35); // 减少到35%
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    // 限制在15%到75%之间，给响应更多空间
    const clampedHeight = Math.max(15, Math.min(75, newHeight));
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full select-none">
      {/* Upper Section - Request Configuration - 更紧凑 */}
      <div 
      ...
      >
        ...
        {/* Request URL Bar - 减少内边距 */}
        <div className="px-3 py-2 border-b border-gray-200 flex-shrink-0">
          <div className="flex space-x-2">
            <select
              value={tab.method}
              onChange={(e) => handleMethodChange(e.target.value)}
              className="px-2 py-1.5 border border-gray-300 rounded text-sm font-medium focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white"
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="bg-blue-600 text-white px-4 py-1.5 rounded hover:bg-blue-700 transition-colors flex items-center space-x-1 disabled:opacity-50 text-sm"
            >
              <Send size={14} />
            ...
            <button 
              ...
              className="p-1.5 text-gray-400 hover:text-yellow-500 border border-gray-300 rounded transition-colors"
        ...
        {/* Request Configuration Tabs - 减少内边距 */}
        ...
        {/* Request Configuration Content - 减少内边距 */}
        <div className="flex-1 overflow-y-auto">
          <div className="p-3">
            {activeSection === 'params' && (
              <div>
                <h3 className="text-xs font-medium text-gray-900 mb-2">Query Parameters</h3>
            ...
            {activeSection === 'headers' && (
              <div>
                <h3 className="text-xs font-medium text-gray-900 mb-2">Headers</h3>
                <div className="space-y-2">
            ...
            {activeSection === 'body' && (
              <div>
                <h3 className="text-xs font-medium text-gray-900 mb-2">Request Body</h3>
                <textarea
                  value={tab.body || ''}
                  onChange={(e) => updateTab(tabId, { body: e.target.value })}
                  placeholder="Enter request body (JSON, XML, etc.)"
                  className="w-full h-24 px-2 py-1.5 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono text-xs"
                />
              </div>
            )}
            ...
            {activeSection === 'auth' && (
              <div>
                <h3 className="text-xs font-medium text-gray-900 mb-2">Authentication</h3>
      ...
      {/* Resizable Divider - 更小 */}
      <div 
        className={`flex items-center justify-center h-1 bg-gray-200 border-t border-b border-gray-300 cursor-row-resize hover:bg-gray-300 transition-colors ${
      ...
      >
        <div className="flex-1 p-2 min-h-0">
          {isLoading ? (
            <div className="bg-white border border-gray-200 rounded-lg p-6 text-center text-blue-600 h-full flex flex-col items-center justify-center">
              <div className="animate-spin w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full mb-3"></div>
              <span className="text-base">Sending request...</span>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 text-center text-gray-500 h-full flex flex-col items-center justify-center">
              <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mb-3">
                <svg className="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
              </div>
              <span className="text-base font-medium">Send a request to see the response</span>
              <span className="text-xs text-gray-400 mt-1">Response will appear here with Monaco Editor</span>
            </div>
          )}
        </div>
      </div>
